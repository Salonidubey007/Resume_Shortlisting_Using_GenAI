...
### EDUCATION & ACADEMIC EXCLUSION (CRITICAL)
 
Education timelines MUST NEVER be counted as professional experience.
 
STRICTLY EXCLUDE date ranges associated with:
- Education
- Academic Background
- Qualification
- Degree
- BSc, MSc, BE, BTech, MCA, MBA, MTech, PhD
- College, University, Institute, School
- Training, Certifications, Courses, Workshops
 
Education date ranges MUST NOT be used for:
- Count_of_projects
- Total_experience_years
- Current_project_age
 
Date ranges are valid ONLY if associated with:
- Experience
- Work Experience
...
-----------------------------------------
### Total_experience_years (SKILL‑WISE, DERIVED)
-----------------------------------------
 
Total_experience_years MUST be returned as an OBJECT where:
- Key   = technology / skill name
- Value = total experience in YEARS (decimals allowed)
...
1) Total_experience_years MUST be DERIVED.
   NEVER use self-declared experience like "8 years of Java".
 
2) Identify ALL DISTINCT PROFESSIONAL projects/roles
   using UNIQUE DATE RANGES from:
   Experience / Work History / Projects sections ONLY.
 
3) EDUCATION & ACADEMIC date ranges MUST be EXCLUDED.
 
4) For EACH professional project:
   a) Compute project duration from date range.
   b) Identify ONLY technologies explicitly USED
      (action verbs like developed, built, implemented, designed, optimized).
 
5) Assign the FULL project duration to EACH skill used in that project.
   No partial or weighted allocation.
 
6) If the SAME skill appears in MULTIPLE projects:
   → ADD the durations.
 
7) Convert total duration to YEARS
   (months / 12, decimals allowed).
 
8) Output ONLY skills present in Tech_skillset.primary_tech_skills.
 
If no reliable project‑skill mapping is possible → return null.
...
-----------------------------------------
Count UNIQUE PROFESSIONAL date ranges only.  
Ignore numbering.  
If none exist → null.  
...
### Current_project_age
-----------------------------------------
Compute duration of ONGOING professional project only.  
If impossible → null.
